...
interface ParkingStrategy {
  findFloorAndSpot(floors: Floor[], vehicleType: 2 | 4): { floorIndex: number; spot: { row: number; column: number } } | null;
}
interface Helper {
...
class LowestIndexFirstStrategy implements ParkingStrategy {
  findFloorAndSpot(floors: Floor[], vehicleType: 2 | 4): { floorIndex: number; spot: { row: number; column: number } } | null {
    for (let i = 0; i < floors.length; i++) {
      const freeSpots = floors[i].getFreeSpots(vehicleType);
      if (freeSpots.length > 0) {
        return { floorIndex: i, spot: freeSpots[0] };
      }
    }
    return null;
  }
...
class MostFreeSpotsStrategy implements ParkingStrategy {
  findFloorAndSpot(floors: Floor[], vehicleType: 2 | 4): { floorIndex: number; spot: { row: number; column: number } } | null {
    let maxFreeSpots = -1;
    let bestFloorIndex = -1;

    for (let i = 0; i < floors.length; i++) {
      const freeCount = floors[i].countFreeSpots(vehicleType);
    ...
    if (bestFloorIndex !== -1) {
      const freeSpots = floors[bestFloorIndex].getFreeSpots(vehicleType);
      if (freeSpots.length > 0) {
        return { floorIndex: bestFloorIndex, spot: freeSpots[0] };
      }
    }
...
class ParkingLot {
  ...
  park(
    ...
    const result = strategy.findFloorAndSpot(this.floors, vehicleType);
    if (result) {
      const { floorIndex, spot } = result;
      const spotId = `${floorIndex}-${spot.row}-${spot.column}`;
      this.floors[floorIndex].parkVehicle(vehicleType, vehicleNumber, ticketId, spot);
      this.helper.println(`Parked at: ${spotId}`);
      return spotId;
    }

    this.helper.println("No parking spot available");
    return "";
  }
...
class Solution {
  ...
  park(vehicleType: 2 | 4, vehicleNumber: string, ticketId: string,  strategy: ParkingStrategy): string {
    return this.parkingLot.park(vehicleType, vehicleNumber, ticketId, strategy);
  }
...
const strategy1 = new LowestIndexFirstStrategy();
const strategy2 = new MostFreeSpotsStrategy();

const spot1 = solution.park(4, "MH12AB1234", "T123", strategy1); // Strategy 0
console.log("Assigned Spot:", spot1);

const spot2 = solution.park(4, "DL12XY5678", "T456", strategy2); // Strategy 1
